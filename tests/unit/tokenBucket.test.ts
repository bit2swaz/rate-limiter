...
describe('tokenBucket', () => {
  ...
  it('allows exactly capacity requests then rejects', async () => {
    ...
    expect(results.slice(0, capacity).every((r) => r.allowed === 1)).toBe(true);
    expect(results[capacity].allowed).toBe(0);
  });
  ...
  it('refills tokens over elapsed time', async () => {
    ...
    expect((await tokenBucket(redis, key, capacity, refillRate, t0)).allowed).toBe(0);

    // simulate 1 second passing
    const t1 = t0 + 1000;
    expect((await tokenBucket(redis, key, capacity, refillRate, t1)).allowed).toBe(1);
  });
  ...
  it('partial refill works (less than 1 full token should not allow)', async () => {
    ...
    const t1 = t0 + 200;
    expect((await tokenBucket(redis, key, capacity, refillRate, t1)).allowed).toBe(0);

    // 600ms elapsed => 1.2 tokens earned, enough
    const t2 = t0 + 600;
    expect((await tokenBucket(redis, key, capacity, refillRate, t2)).allowed).toBe(1);
  });
